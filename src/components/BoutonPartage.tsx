 ...
 *
 * Takes the link already built rather than the state, so the serialisation
 * stays in one place.
 */
  ...
  return (
    <div className="mt-4">
      ...
      >
        {etat === 'copie' ? (
          <>
            <span className="text-brand-600">✓</span> {t.partage.copie}
          </>
        ) : (
          t.partage.copier
        )}
      ...
      <p className="mt-2 px-2 text-xs leading-relaxed text-ink-400">
        {etat === 'manuel' ? t.partage.noteManuelle : t.partage.note}
      </p>
    </div>
  );
}
